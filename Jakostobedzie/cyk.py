...
s = 1 #float(input("Podaj wartość spinu: "))
spin = vec_spin(s)
N = 6
nmini = int((int(N/2))-1)
potega = int(N-nmini)
...
for i in range(200):
    ...
    ind = np.argsort(np.real(wm))[:tamto:-1] #może byc źle (15)
    T = np.array([vm[:,k]for k in ind]).T
    T2 =np.conjugate(T.T)
    ...
    wm, vm = np.linalg.eig(matrixy)
    ind = np.argsort(np.real(wm))[:tamto:-1] # moze być źle (15)
    T = np.array([vm[:,k]for k in ind]).T
    T2 =np.conjugate(T.T)
    y = np.matmul(np.matmul(T2,ham4),T)
    plik.write(str(i+1)+". "+str(temp[1]/(N+(2*i)))+"\n")
    plik.flush()
    #print(temp[1]/(N+((din)*i)))
plik.close()
min_energy(x,y)
